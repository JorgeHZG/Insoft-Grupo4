...
public class PlazaDAO {
	...
	/*
	 * private static String consultarPlazaOcupada =
	 * "SELECT  myparking.\"Estancia\".\"Num_plaza\", "
	...
	/*
	 * private static String modificarPlazaNOOcupada =
	 * "UPDATE myparking.\"Plaza\" SET \"estado\" = false WHERE \"Numero\" = ?";
	 */

	/*
	 * private static String anadirPlaza =
	 * "INSERT INTO  myparking.\"Plaza\" VALUES (?, ?, ?)";
	 */
	...
	public PlazaVO Aparcar(String tipo) throws SQLException {
		try (Connection conn = ConnectionManager.getConnection()) {
			// Abrimos conexión
			PlazaVO plaza = new PlazaVO(-1, false, tipo);
			try (PreparedStatement statement = conn.prepareStatement(consultarEstadoPlaza)) {
	...
	/*
	 * public PlazaVO SalirParking(String matricula) throws SQLException {
	 * try (Connection conn = ConnectionManager.getConnection()) {
	 * // Abrimos conexión
	 * PlazaVO plaza = new PlazaVO(-1, false, tipo);
	 * try (PreparedStatement statement =
	 * conn.prepareStatement(consultarEstadoPlaza)) {
	 * // Asignamos prarametros
	 * statement.setString(1, tipo);
	 * System.out.println(statement.toString());
	 * // Ejecutamos la consulta
	 ...
	 * } else {
	 * System.out.
	 * println("No se ha encontrado una plaza libre para el tipo de vehiculo seleccionado"
	 * );
	 * return plaza; // Si no ha habido exito, se devuelve una plaza con numero "-1"
	 * }
	 * } catch (SQLException se) {
